...
public class PrenoEntity extends AbstractDiggingDino{

    private static final EntityDataAccessor<Integer> DATA_REMAINING_ANGER_TIME = SynchedEntityData.defineId(PrenoEntity.class, EntityDataSerializers.INT);
    ...
    public PrenoEntity(EntityType<? extends TamableAnimal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob pOtherParent) {
        PrenoEntity baby = ModEntityTypes.PRENO.get().create(serverLevel);
        PrenoVariant variant = Util.getRandom(PrenoVariant.values(), this.random);
    ...
    public PrenoVariant getVariant() {
        return PrenoVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    private void setVariant(PrenoVariant variant) {
